...
const Quanlytaikhoan = () => {
  ...
  const handleReviewToggle = (productId: number | string, currentStatus: boolean) => {
    if (confirm('Bạn có chắc chắn muốn thay đổi trạng thái Người Dùng không?')) {
            ...
            <nav aria-label="breadcrumb" className="w-max">
              <ol className="flex flex-wrap items-center w-full bg-opacity-60 rounded-md bg-transparent p-0 transition-all">
                ...
                <li className="flex items-center text-blue-900 antialiased font-sans text-sm font-normal leading-normal cursor-pointer transition-colors duration-300 hover:text-blue-500">
                  <p className="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-normal">Tài Khoản</p>
                </li>
              </ol>
            </nav>
            <h6 className="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900">Tài Khoản Người Dùng</h6>
          </div>
        </div>
      </nav>
                          ...
                          {products.map((product: IProduct, index: number) => (
                            <tr key={product.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                              ...
                              <td className="py-4 px-6 text-sm text-left font-medium whitespace-nowrap">
                                ...
                                >
                                  {product.isReviewed ? 'Hoạt Động' : 'Đã khóa'}
                                </button>
                              </td>
                            </tr>
                          ))}
