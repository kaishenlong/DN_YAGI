        ...
        $thongKeChiNhanh = DB::table('hotels')
            ...
            ->addSelect([
                ...
                'ti_le_huy' => DB::table('bookings')
                    ...
                    ->whereColumn('detail_rooms.hotel_id', 'hotels.id')
                    ->where('bookings.status', 'cancelled')
                    ->selectRaw('(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM bookings INNER JOIN detail_rooms ON bookings.detail_room_id = detail_rooms.id WHERE detail_rooms.hotel_id = hotels.id)) as ti_le_huy')
                    ->limit(1),
     ...
     SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12) AS month_range'))
        ...
        $thongKeKhachHang = DB::table('users as u')
            ->select(
                ...
                DB::raw('COUNT(b.id) as so_lan_dat_phong'),
                DB::raw('COUNT(CASE WHEN b.status = "confirmed" THEN 1 END) as so_lan_dat_phong_thanh_cong'),
                DB::raw('(COUNT(CASE WHEN b.status = "confirmed" THEN 1 END) * 100.0 / COUNT(b.id)) as ty_le_dat_phong_thanh_cong'),
                DB::raw('(COUNT(CASE WHEN b.status = "cancelled" THEN 1 END) * 100.0 / COUNT(b.id)) as ty_le_huy_dat_phong'),
