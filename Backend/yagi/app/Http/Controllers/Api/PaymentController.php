<?php
    ...
    public function PayCart(Request $request){
        if (!Auth::check()) {
            return ['error' => 'User not logged in', 'status_code' => 401];
        }

        $userId = Auth::id();

        // Lấy giỏ hàng của người dùng từ Cart
        $cart = Cart::find($request->cart_id);
        if (!$cart) {
            return ['error' => 'Cart not found', 'status_code' => 404];
        }

        // Lấy các sản phẩm trong CartDetail
        $cartDetails = $cart->cartDetails;
        if ($cartDetails->isEmpty()) {
            return ['error' => 'Cart is empty', 'status_code' => 400];
        }

        // Tính tổng giá trị thanh toán từ giỏ hàng
        $totalPrice = 0;
        foreach ($cartDetails as $cartDetail) {
            $room = DetailRoom::find($cartDetail->detail_room_id);
            if ($room) {
                $totalPrice += $room->into_money * $cartDetail->quantity;
            }
        }
        ...
        switch ($request->method) {
            case 'MoMo':
                $payment->method = 'MoMo';
                $redirectUrl = "https://momo.vn/payment?amount={$totalPrice}&cart_id={$cart->id}";
                break;
            ...
            case 'QR':
                $payment->method = 'QR';
                $redirectUrl = "https://qrpayment.vn/pay?amount={$totalPrice}&cart_id={$cart->id}";
                break;

            default:
                return ['error' => 'Invalid payment method', 'status_code' => 400];
        }
